...
//
#pragma db view query("SELECT first, last, age FROM test_view_person")
struct view1
{
  std::string first;
  std::string last;
...
//
#pragma db view table("test_view_person")
struct view1d
{
  #pragma db column("first")
  std::string first;
...
#pragma db view object(person) \
  query("SELECT count(id) FROM test_view_person")
struct view2
{
  std::size_t count;
};
...
struct view2b
{
  #pragma db column("count(test_view_person.id)")
  std::size_t count;
};
...
#pragma db view object(person) object(country = residence)  \
  query("SELECT first, last, residence.name "               \
        "FROM test_view_person "                            \
        "LEFT JOIN test_view_country AS residence "         \
        "ON test_view_person.residence = residence.code")
struct view4
{
...
//
#pragma db view table("test_view_person" = "p")                   \
  table("test_view_employer_employees" = "ee": "ee.value = p.id") \
  table("test_view_employer" = "e": "ee.object_id = e.name")
struct view6c
{
